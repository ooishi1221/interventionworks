...
**ライダーの全体験を写真1枚で刻む地図。** バイク駐車場の検索アプリではない。停めた場所、食った飯、見た景色 — ライダーがパシャッと撮った1枚をAIが判別し、自分のノートに残り、誰かの地図になる。
...
2. **ワンショット（写真1枚）** — 到着したら写真を1枚撮るだけ。それが自分の駐車ノートになり、副産物としてスポットの鮮度が更新される。「報告」ではなく「自分のメモ」。撮った写真は自分のノート（RiderScreen）にも、スポット詳細にも残る
...
- 📸 ワンショット撮影（任意） — 写真1枚で自分のノートに保存 + スポット鮮度が副産物として更新
- 完了 → 鮮度（lastVerifiedAt）更新 + 駐車履歴に記録
...
**到着時の自動処理（ボタン不要）:**
- `reportParked()` → Firestore鮮度更新（`lastVerifiedAt` + `currentParkedAt`）
...
3. 50m以内到着 → **自動で鮮度更新 + 足跡記録 + 駐車セッション開始** → 通知
...
- **ニアバイ（≤50m）**: 「ここに停めた？メモしとく」カード
  - 停めた → 📷ワンショット撮影（任意）→ 自分のノートに保存 → 鮮度更新
  - 停められなかった → **即記録 + 近くの別の場所リスト表示**（理由チップは下部に「任意」配置）
- **スポットなし（表示圏内0件）**: 「新しい場所を見つけた？」/ 「他を探す」→「アプリ内広域検索」or「Googleマップ」分岐
- **通常（上記以外）**: カード非表示
- クールダウン: AsyncStorage `lastReport_{spotId}` で同一スポット2h以内は抑制（自動出発と同期）
...
スポットの鮮度を `lastVerifiedAt`（= lastConfirmedAt）の経過時間で3段階に表示。ワンショット撮影や到着検知で `reportParked()` が呼ばれると副産物として更新される。

| 鮮度 | 経過時間 | 色 | 透明度 |
|------|---------|-----|--------|
| clear（確認済み） | 1ヶ月以内 | 緑 `#30D158` | 1.0 |
| hazy（やや古い） | 3ヶ月以内 | アンバー `#FF9F0A` | 0.6 |
| foggy（未確認） | それ以上 | グレー `#48484A` | 0.35 |

- `currentParked > 0` のピンは台数バッジを表示
- マーカーはsource（seed/user）で区別しない。鮮度の色+透明度で表現
- 広域ズーム時のドット: 24×24px + 白stroke 2px（視認性確保）
- 到着検知で自動書き込み（`reportParked` → Firestore `lastVerifiedAt` + `currentParkedAt` 更新）
...
| 画面 | 概要 |
|------|------|
| **MapScreen** | メイン地図。クラスタリング付きピン表示、タップで詳細シート。近接コンテキストカード表示 |
| **RiderScreen** | 足跡地図（自分が停めた場所が光る地図）+ 日記タイムライン（タップでマップジャンプ）+ 自分のノート（写真ギャラリー）+ 駐車中カード（表示のみ、出発は自動） |
...
### ナビゲーション（タブバー 3アイテム: マップ / ワンショット / 設定）

| 位置 | 画面 | アイコン |
|------|------|----------|
| 左 | MapScreen | `map` |
| **中央（突き出し）** | **ワンショット撮影** | `camera`（64pt丸、オレンジ `#FF6B00`） |
| 右 | SettingsScreen（→ お問い合わせ・利用規約サブ画面） | `settings` |

**ライダー画面への導線:** ピルバー左端のアバターアイコン（足跡数バッジ付き）→ タップで RiderScreen をオーバーレイ表示。MapScreen はアンマウントされず位置保持。
...
- **近接コンテキストカード（下部）** — 現在地と最寄りスポットの距離で自動表示。≤50mで「停めた→📷ワンショット（任意）」「停められなかった→理由（スキップ可）→近くの別の場所」（現在検索）。2hクールダウン付き。スポット0件時は「アプリ内広域検索」or「Googleマップで探す」分岐
- **Googleマップ検索復帰** — Googleマップ遷移時にセッション保存 + 15分後リマインダー通知。AppState復帰検知で「見つかった？登録する？」カード表示（#137 実機検証待ち）
- **エリア自動再検索** — 地図を移動すると表示範囲の30%以上移動時に自動でスポット再取得（デバウンス800ms）
- スポット詳細シート表示中はピルバー・近接カードを非表示（ワンショットボタンはタブバーに常駐）
...
| 足跡の残し方 | 「停めた✓」ボタン | ワンショット撮影（写真1枚が自分のノートになる） |
...
| LiveFeed（ライブフィード） | 廃止完了。ワンショット+自分のノートが代替 |
| 温度システム（temperature.ts） | 廃止完了。鮮度システム（freshness.ts）に置換 |
| 「停めた ✓」ボタン | 廃止完了。到着検知の自動記録 + ワンショット撮影が代替 |
...
| 機能 | 優先度 | 状態 |
|------|--------|------|
| ~~到着通知~~ | ~~P0~~ | **実装済み** — `useArrivalDetection` + 自動鮮度更新 + 足跡記録 |
| ~~「ここ行く」マーク~~ | ~~P0~~ | **実装済み** — SpotDetailSheet「案内開始」で自動セット |
| ~~料金・営業時間・決済手段フィールド~~ | ~~P1~~ | **実装済み** — priceInfo + SpotPayment + SpotDetailSheet表示 |
| ~~足跡地図（RiderScreen刷新）~~ | ~~P1~~ | **実装済み** — v4: 足跡マップ + 日記タイムライン（タップでマップジャンプ）+ 駐車中カード（表示のみ、出発ボタン廃止） |
...
| ~~ワンショット撮影~~ | ~~P0~~ | **実装済み** — `pickPhotoFromCamera` → `addReview` → `reportParked` で写真1枚が自分のノート+スポット鮮度更新 |
| ~~自分のノート~~ | ~~P1~~ | **実装済み** — SpotDetailSheet写真表示 + RiderScreen写真ギャラリー（`fetchUserPhotos`） |
