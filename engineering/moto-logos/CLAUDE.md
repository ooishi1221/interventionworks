...
**ポジショニング:** 「都市ライダーの駐車場マップ」ではなく、**「停められた」が見える地図。** 同じ排気量のライダーが「ここに停めた」という生の足跡が地図に載っている — これがGoogleにもNAVITIMEにもない非対称優位。

**将来の拡張（Phase 3以降）:** カフェ・飯屋、ツーリングルート、景色スポット、注意ポイント（路面・取締り）等。ビジョンは「ライダーの足跡が残る地図」であり、駐車場はその最初のカテゴリに過ぎない。コンセプトは広く、プロダクトは狭く。
...
- マーカーはsource（seed/user）で区別しない。鮮度の色+透明度で霧を表現
...
- **ピルバー（上部）** — ダークグラスのフローティングバー。左にCCフィルタチップ、中央に最寄り1件（タップで展開→3件リスト）、右に🔍検索ボタン（未来検索の入口）。検索結果表示中はエリアサマリーモード（「エリア名 — Nスポット」+ ×クリアボタン）に切り替わる。現在地復帰はマップタブ2度押しで対応（スポット再取得も同時実行）
...
- **「駐車場マップ」と言わない。「停められた」が見える地図。** 機能ではなく体験を語る
