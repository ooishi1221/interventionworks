...
- **最寄りスポットリスト（上部）** — ライブフィード直下に近い順3件を常時表示。ナビ1タップ起動。タップでピンにジャンプ
- **FABオレンジボタン（右下）** — タップで2択メニュー展開（文字で探す / 場所を登録）。登録は写真1枚で完了
- **現在地ボタン（右下、FAB上）** — タップで現在地にアニメーション移動
...
- スポット詳細シート表示中はFAB・最寄りリスト・近接カードを非表示
- ~~ラジアルメニューは廃止~~（機能は FAB・最寄りリスト・自動再検索・現在地ボタンに分散）
