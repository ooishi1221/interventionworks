...
- 📸 ワンショット撮影（任意） — 写真1枚でライダーノートに保存 + スポット鮮度が副産物として更新
...
- **ニアバイ（≤50m）**: 「ここに停めた？メモしとく」カード
  - 停めた → 📷ワンショット撮影（任意）→ ライダーノートに保存 → 鮮度更新
...
| **RiderScreen** | ライダーノート（タブ）: ヒーローカード（バイク写真+ニックネーム）+ ワンショットカード（写真+場所名+粗粒度時間+AI分析バッジ）+ 足跡サマリー（カウント+小地図） |
...
- **近接コンテキストカード（下部）** — 現在地と最寄りスポットの距離で自動表示。≤50mで「ワンショット」ボタン表示。2hクールダウン付き。スポット0件時は「アプリ内広域検索」or「Googleマップで探す」分岐
...
| 「停められなかった」フロー | 廃止完了。近接カードからボタン・理由チップ・候補リスト全削除 |
...
| ~~足跡地図（RiderScreen刷新）~~ | ~~P1~~ | **実装済み** — v6: タブ化 + ワンショットカード（写真ファーストダッシュボード）+ 足跡サマリー + AI分析バッジ（プレースホルダー） |
...
| ~~ワンショット撮影~~ | ~~P0~~ | **実装済み** — `pickPhotoFromCamera` → `addReview` → `reportParked` で写真1枚がライダーノート+スポット鮮度更新 |
