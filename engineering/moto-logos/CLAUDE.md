...
| 温度 | 経過時間 | 色 |
|------|---------|-----|
| blazing | 30分以内 | 赤 `#FF3B30` |
| hot | 2時間以内 | オレンジ `#FF6B00` |
| warm | 6時間以内 | アンバー `#FF9F0A` |
| cool | 24時間以内 | 水色 `#64D2FF` |
| cold | それ以上 | グレー `#8E8E93` |

- 脈動アニメーションは廃止（バッテリー/発熱対策）。色のみで温度を表現
- `currentParked > 0` のピンは台数バッジを表示
- マーカーはsource（seed/user）で区別しない。温度のみで色分け（source区別は詳細シート内のバッジに留める）
- 広域ズーム時のドット: 24×24px + 白stroke 2px（視認性確保）
- 到着検知で自動書き込み（`reportParked` → Firestore `currentParkedAt` 更新）
- ライダーの行動負担: **ゼロ**。到着=停めたと推定。誤差は2h自動出発で吸収
