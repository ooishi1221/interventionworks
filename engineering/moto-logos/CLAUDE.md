...
2. **呼吸する霧（Fog of War）** — 霧鮮度システム。ライダーが「停めた」を確認するとスポットの霧が晴れ、時間経過で霧が戻る。晴れたピン = 最近確認済みで信頼できる。霧のピン = 未確認、あなたが晴らせる。地図全体が「ライダーコミュニティの呼吸」になる
...
- **ニアバイ（≤50m）**: 「ここに停めた？メモしとく」カード
  - 停めた → 📸写真メモ（任意、タグ選択なしで即アップロード）→ 完了 → 鮮度更新
...
### 霧鮮度システム — Fog of War（地図ピン）

ライダーが「停めた」を確認するとスポットの霧が晴れる。時間経過で霧が戻る。`lastVerifiedAt`（= lastConfirmedAt）の経過時間で3段階。
...
- 「停めた ✓」ボタンで手動確認（`confirmSpot` → Firestore `lastVerifiedAt` 更新）
- LiveFeedは廃止。地図の霧の動きが代替
...
| **RiderScreen** | 足跡地図（自分が停めた場所が光る地図）+ 日記タイムライン（タップでマップジャンプ）+ 駐車中カード（表示のみ、出発は自動） |
...
- **近接コンテキストカード（下部）** — 現在地と最寄りスポットの距離で自動表示。≤50mで「停めた→📸看板メモ」「停められなかった→理由（スキップ可）→近くの別の場所」（現在検索）。2hクールダウン付き。スポット0件時は「アプリ内広域検索」or「Googleマップで探す」分岐
...
| ~~駐車温度システム~~ | — | **実装済み** — 5段階温度（色のみ、脈動廃止）+ 到着自動記録 |
