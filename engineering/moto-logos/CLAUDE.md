...
2. **仲間の気配** — ライブフィードは焚き火。内容より「動いている」ことが大事。鮮度バッジ=「最近誰かがいた証」。鮮度で色分け（1ヶ月以内=青、3ヶ月=黄、6ヶ月以上=赤）
...
├── db/             # SQLite スキーマ・CRUD
├── hooks/          # カスタムフック（useDatabase, useProximityState, useArrivalDetection）
...
3. 50m以内到着 → ローカル通知 → 行き先クリア
4. 通知タップ → アプリ復帰 → 近接カード（`useProximityState`）が発動
...
- `updatedAt` に基づいてピンの色を自動変更:
  - 1ヶ月以内 → 青 `#0A84FF`（最近誰かがいた）
  - 3ヶ月以内 → 黄 `#FFD60A`（しばらく誰も来ていない）
  - 6ヶ月以上 → 赤 `#FF453A`（足跡が途絶えている）
...
| **RiderScreen** | 足跡地図（自分が停めた場所が光る地図）+ 活動タイムライン → **刷新予定** |
...
| 機能 | 優先度 | 概要 |
|------|--------|------|
| ~~到着通知~~ | ~~P0~~ | **実装済み** — `useArrivalDetection` フック。フォアグラウンドGPS + ローカル通知 |
| ~~「ここ行く」マーク~~ | ~~P0~~ | **実装済み** — SpotDetailSheet「案内開始」で自動セット |
| 料金・営業時間・決済手段フィールド | P1 | Firestore spots に追加。ライダーが行く前に知りたい情報 |
| 看板OCR | P1 | 写真から料金・営業時間を自動抽出 |
| 足跡地図（RiderScreen刷新） | P1 | 数字のダッシュボードではなく、自分が停めた場所が光る地図 |
| 駐車履歴の自動記録 | P1 | SQLite parking_sessions を活用 |
| コールドスタート用データ投入 | P1 | 主要エリアのスポットを一括登録 |
| 「あなたの足跡で○人が助かった」通知 | P2 | デジタルヤエー |
| カメラロールからの写真選択 | P2 | 後からでも写真を貼れる |
| 入口写真のタグ分け | P2 | 看板写真と入口/外観写真を区別 |
