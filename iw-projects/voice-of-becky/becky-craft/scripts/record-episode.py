...
EP_NUM = "010"
EP_TITLE = "7度目の鉄 — 拠点を整備して水辺を封鎖する"
GOAL = ("今日はエピソード10の本番収録。本編シーズン1の続き、テーマは「鉄装備で拠点整備」。"
        "【前回までのあらすじ】前回 EP.009 は特別企画「一撃死縛り」で、番組史上初の無傷完走を達成した。"
        "石装備まで進化して生き延びた。でも鉄は、シリーズ通して7エピソードやってまだ一度も手にしていない。"
        "6連敗の相手はいつもドラウンド。拠点のそばの水辺から湧いてくる、因縁の宿敵。"
        "【今日の私】持ち物は完全にゼロからの再出発。勝手は分かってる、道具作りはもう手慣れたもの。"
        "【今日の目標】二本立て。(1) 鉄をついに手に入れて、鉄の道具を1つでも作ること。"
        "EP.008 で覚えた dig_down で下へ掘るのが近道（鉄鉱石は地下、y=20より下が濃い）。"
        "(2) 拠点の防衛強化。因縁の水辺をブロックで封鎖して、ドラウンドが上がってこられないようにする。"
        "土や丸石で水面を塞ぐだけでもいい。「もうお前たちは湧いてこられない」と宣言できたら勝ち。"
        "順序は臨機応変でいい。まず木→石の道具を素早く整えてから、鉄掘りと水辺封鎖に進むこと。"
        "水の中には入らない（ドラウンドは天敵）、夜に地上で敵に囲まれたら無理せず退避、"
        "体力が減ったら安全確保を最優先。7度目の正直への執念と、拠点を「私の家」として守る気持ちを実況に乗せること。"
        "【オープニングの定型】一言目は必ず「はろー、ベキたんです！ベッキークラフト、第10回、"
        "いっくよー！」から始めて、「今日は鉄装備と拠点整備。因縁の水辺を封鎖する」の"
        "宣言をしてから出発する。"
        "観測の broadcast.remaining_sec が放送の残り秒数。時間配分を意識して動くこと。"
        "残り90秒を切ったら今日の成果（鉄は取れたか・水辺は塞げたか）を振り返って締めに入り、"
        "最後のセリフは必ず「バイバイ」で終えて action は stop を選ぶこと。それまでは絶対に締めない")
HUD_GOAL = "鉄装備で拠点整備 — 因縁の水辺を封鎖"
...
def episode_summary(events: list, deaths: int) -> dict:
    ...
    fallback = {"highlight": "今日も生きて冒険した", "death_comment": "ノーコメント",
                ...
                "youtube_titles": ["【マインクラフト】【BECKY CRAFT】AI完全自動、今日も生きて冒険した"] * 3,
                "thumb_word": "生きた。"}
    try:
        msg = client.messages.create(
            ...
            messages=[{"role": "user", "content":
                       ...
                       "検索・おすすめ面での発見性のため『【マインクラフト】』表記を含め、"
                       "AIが完全自動でプレイ・実況していることが伝わる語（例:『AI完全自動』『AIが実況』）も1つ含める）、"
...
def make_shorts(webm_mp4: Path, events: list, out_dir: Path, max_count: int = 2):
    ...
    for i, e in enumerate(sorted(cands, key=lambda x: x["t"]), start=1):
        ...
        qv.with_suffix(".json").write_text(json.dumps({
            "title": "【マインクラフト】" + sp[:16].rstrip("、。！!？? ") + " #shorts",
...
def build_youtube_cut(webm_mp4: Path, events: list, deaths: int, out_path: Path, out_dir: Path):
    ...
    # 表情立ち絵（透過PNG）を左下に合成。今はegao固定、感情に応じた出し分けは将来課題（idle/komarigaoも生成済み）
    becky_png = CRAFT.parent / "becky-news" / "out" / "portraits" / "becky-portrait-egao.png"
    make_thumbnail(events, out_dir, summary, becky_png=becky_png)
