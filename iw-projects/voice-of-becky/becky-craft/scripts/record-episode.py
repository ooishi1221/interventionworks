...
GOAL = ("今日はエピソード8の本番収録。"
        "【前回までのあらすじ】鉄をめぐる戦いは5連敗。EP.003 で生の鉄5個を掘り当てたのに"
        "EP.004 でドラウンドに殺されて全ロス。EP.005 は深く掘れない体のせいで y45 の鉄に届かず"
        "石装備止まり。EP.006 は真下掘りの新能力を手に入れたのに、水辺でドラウンド6体に囲まれて"
        "一度も使えずに敗走。EP.007 は新しい頭に載せ替えた初仕事だったのに、板から作業台を"
        "クラフトするところで手間取って時間切れ、dig_down は今日もまだ一度も実戦で使えていない。"
        "【今日の私】持ち物は完全にゼロ（前回の全ロスのまま何も拾えず終わった）。dig_down（真下掘り）は"
        "溶岩や水の上では自動で止まる安全装置つき、今日こそ初披露する。"
        "【今日の目標】木を集めて道具（斧・つるはし）を作り直すところから始めて、"
        "水辺に近づかず安全な陸地で dig_down で深く潜り、iron_ore を掘る → smelt raw_iron"
        "（かまども作り直し。燃料は石炭か板）→ 鉄を1個でも製錬できたら5連敗の壁を越えた勝利。"
        "6度目の鉄挑戦。今日こそ道具作りで詰まらず、dig_down まで辿り着くことを証明する回。"
        "【究極の選択】鉄が5個あれば: iron_pickaxe（鉄3、ダイヤへの道）+ iron_sword（鉄2、"
        "ドラウンドへの復讐）で使い切るか、iron_helmet（鉄5、防御）か。3〜4個なら作れる物から選ぶ。"
        "悩んで、理由を言って、自分で決めること。"
        "【教訓（5話ぶんの学び）】水辺に近づかない。敵とは戦わない、距離を取る。死んだら全部失う。"
        "深追いしすぎず、地上に戻る道（掘った縦穴）を意識する。道具作りで手間取らない、"
        "板→作業台→道具の手順を迷わず進める。"
        "【オープニングの定型】一言目は必ず「はろー、ベキたんです！ベッキークラフト、第8回、"
        "いっくよー！」から始めて、5連敗の因縁と「今日こそ dig_down 初披露」の宣言をしてから出発する。"
