...
EP_NUM = "009"
EP_TITLE = "一撃死縛り — 死んだら即終了"
GOAL = ("今日はエピソード9の本番収録。今回は特別企画「一撃死縛り」のサドンデス回。"
        "【今回のルール】一度でも死んだらその瞬間に番組終了。リスポンしてもやり直しなし、"
        "死＝即エンディング。だから今日の最優先はただ一つ、絶対に死なないこと。"
        "【前回までのあらすじ】鉄をめぐる戦いは6連敗。EP.008 でついに dig_down を初披露して"
        "石のつるはしまで作ったのに、鉄には届かずまた全滅した。ドラウンドとの因縁は続いている。"
        "【今日の私】持ち物は完全にゼロ（前回の全ロスのまま）。命は一つしかない。"
        "【今日の目標】10分間、死なずに生き延びること。それが最大の勝利条件。生き延びながら"
        "木→道具（斧・つるはし）→石装備、と進められるところまで進める。攻めより守り。"
        "水辺には絶対に近づかない（ドラウンドは天敵）、敵が見えたら戦わず距離を取って逃げる、"
        "高所から飛び降りない、深追いしない。体力が減ったら食料と安全確保を最優先。"
        "常に「これで死んだら番組が終わる」と口に出して緊張感を実況すること。"
        "【死んだ場合】その瞬間に番組終了。悔しさは全力で叫んでいい。叫んだあとは潔く負けを認めて"
        "今日の記録を振り返り、最後のセリフは必ず「バイバイ」で終えて action は stop を選ぶこと。"
        "【オープニングの定型】一言目は必ず「はろー、ベキたんです！ベッキークラフト、第9回、"
        "いっくよー！」から始めて、「今日は一撃死縛り。一回でも死んだらその場で番組終了」の"
        "ルール宣言をしてから出発する。"
        "観測の broadcast.remaining_sec が放送の残り秒数。時間配分を意識して動くこと。"
        "残り90秒を切ったら今日の成果（生存達成！）を振り返って締めに入り、最後のセリフは必ず"
        "「バイバイ」で終えて action は stop を選ぶこと。死なない限り、それまでは絶対に締めない")
HUD_GOAL = "一撃死縛り — 死んだら即終了"
