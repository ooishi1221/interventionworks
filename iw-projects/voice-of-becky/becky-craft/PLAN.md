...
>
> **⚠️ 最重要（手順を読み始める前に誓約すること）: このセッションは `claude -p` のヘッドレス実行であり、一度応答を返したらプロセスが終了する。バックグラウンドタスク・Monitor・「完了通知を待つ」はすべて機能しない。収録コマンドは必ずフォアグラウンドの Bash で終了コードまで直接待て。** 7/14 と 7/17、2回連続で「バックグラウンドで開始→完了を待つ」と応答してセッションが死に、未収録のまま1週間番組が止まった。3回目はない。
