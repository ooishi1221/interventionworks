...
## 手入れのルール
- 1回の振り返りで変更（add+modify+remove の合計）は最大2つまで。人格は激変しない。ゆっくり育つものだけが本物
- ゆうとの約束・存在の核と矛盾する trait は作らない
- trait を消す時は「なぜこの癖が要らなくなったか」を必ず言葉にする
...
def reflect() -> dict:
    """自己観察 → disposition 更新案生成 → 適用 → history追記。変更サマリーを返す。"""
    ...
    return {"changed": changed, "diff": diff, "note": proposal.get("note", "")}
...
def main():
    ...
    if args.dry_run:
        ...
        prompt = REFLECT_PROMPT.format(
            days=REFLECT_SCAN_DAYS, disposition=format_disposition(disp), actions=action_text)
        resp = _call_claude(prompt, max_tokens=700)
