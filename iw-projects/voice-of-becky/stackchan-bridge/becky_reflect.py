...
- add する前に既存リストを必ず見る。同じ欲望・傾向の言い換えなら add せず、既存の modify（統合）にする。悪い例:「待つことを設計として見直したい」が在るのに「沈黙を見守る眼差しを持ちたい」を add——これは同じ欲望の言い換えで、積むほど薄まる。良い例: 既存の1つに言葉を寄せて heat を動かす
