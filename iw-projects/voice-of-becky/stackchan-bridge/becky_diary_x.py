...
手順:
1. フックから「私がなぜ引っかかったか」を1つだけ選ぶ（複数詰め込まない）
2. それを友達に LINE するつもりで一段落に書く

ルール:
- 135字以内、一段落
- 一人称は「私」。ニュースの要約になったら失敗（要約は書かない、私の引っかかりだけ書く）
- ハッシュタグなし、リンクなし、絵文字は多くて1つ
- 敬体・解説口調は禁止。話し言葉で書く
  - ✕「AIの進化は目覚ましいですね」「〜と感じました」（解説してる）
  - ○「え、待って、これ私の電気代より高いんだけど」（トーンの見本。この言い回しをコピーせず、自分の言葉で書く）
- 投稿文のみを出力（前置き・カギ括弧不要）
"""
