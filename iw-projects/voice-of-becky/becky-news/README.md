...
1. ~~**タイトルに固有名詞（モデル名・企業名）が入ると中央値の 4〜8 倍伸びる**~~（2026-08-03 診断、Kimi 回 1,303 views が根拠）。`auto_news_shorts.py` の `load_unused_news()` で固有名詞マッチ優先選定に反映済み。
   → **⚠️ 2026-08-17 撤回。これは n=1 の過度な一般化だった。** 根拠は Kimi 回1本のみ。8/9 公開の「OpenAIがスタートアップを買収」「Amazonのデータセンター」も同じ固有名詞入りだが **0 views** で、ルールと真正面から矛盾する。
   Kimi 回が伸びた真因は固有名詞ではなく **外部の実ニュースサイクル**である可能性が高い（Moonshot AI「Kimi K3」は 2026-07-16 発表・7月末に350億ドル評価で Bloomberg / The Week が報道した実在の大型ニュース。Studio 実測でも「他のショート動画の 6.9 倍」）。
   選定ロジックを変えるなら「固有名詞が入っているか」ではなく「**そのトピックに外部検索需要が現在あるか**」で判定すべき。`auto_news_shorts.py` は未修正のまま（固有名詞優先が残っている）。
   **教訓: 1本のバイラルから「勝ちパターン」を抽出して自動化に焼き込まない。** 当たった1本は、たいてい中身ではなく世界の側の事情で当たっている
